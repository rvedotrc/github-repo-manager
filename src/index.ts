...
const CONFIG_GITHUB_REPOSITORY_DEFAULT_BRANCH_REF_NAME = "github.repo.defaultbranchref.name";
...
const loadReferenceData = (dir: string): Promise<ReferenceData> =>
    fs.promises.readFile(`${dir}/var/repositories.json`, "utf-8")
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        cp.on("close", (code, signal) => {
            ...
            if (code !== null) {
                ...
                reject(new Error(`git clone received ${signal}`));
            }
        });
    });
...
const ensureConfig = async (key: string, value: string, dir: string, config: Readonly<Record<string, string>>): Promise<void> => {
...
const ensureMetadataPresent = async (repo: Repository, dir: string): Promise<void> => {
    const configText = await new Promise<string>((resolve, reject) => {
        let output = Buffer.of();
        ...
        cp.stdout.on("data", chunk => output = Buffer.concat([output, chunk]));
        cp.on("close", (code, signal) => {
            if (code === null) reject(new Error(`git config killed by ${signal}`));
            if (code !== 0) reject(new Error(`git config exited with ${code}`));
    ...
    const config = pairs.reduce((prev, curr) => ({
        ...prev,
        [curr.key]: curr.value,
    }), {});
    ...
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_OWNER_LOGIN, repo.owner.login, dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_DEFAULT_BRANCH_REF_NAME, repo.defaultBranchRef?.name ?? "", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_VISIBILITY, repo.visibility, dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_URL, repo.url, dir, config);

    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_ARCHIVED, repo.isArchived ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_EMPTY, repo.isEmpty ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_FORK, repo.isFork ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_LOCKED, repo.isLocked ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_MIRROR, repo.isMirror ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_PRIVATE, repo.isPrivate ? "true" : "false", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_IS_TEMPLATE, repo.isTemplate ? "true" : "false", dir, config);

    await ensureConfig(CONFIG_GITHUB_REPOSITORY_CREATED_AT, repo.createdAt, dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_UPDATED_AT, repo.updatedAt ?? "", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_PUSHED_AT, repo.pushedAt ?? "", dir, config);
    await ensureConfig(CONFIG_GITHUB_REPOSITORY_ARCHIVED_AT, repo.archivedAt ?? "", dir, config);
};

const syncAllUnderOwnerToDir = async (owner: string, dir: string): Promise<void> => {
    ...
    for (const repo of remotes.repositories) {
        if (!entries.includes(repo.name)) {
            ...
            console.log(`${dir}/${repo.name} already exists (hopefully it's a clone)`);
            await ensureMetadataPresent(repo, `${dir}/${repo.name}`);
        }
    }
};

syncAllUnderOwnerToDir("rvedotrc", `${process.env.HOME}/github-rvedotrc-all`)
    .catch(err => {
        console.error(err);
        process.exit(1);
    });
